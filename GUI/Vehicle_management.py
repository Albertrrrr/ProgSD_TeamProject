...
tk.Label(top_frame, text="Vehicle Management", font=("Arial", 14, "bold"), bg="#4682B4", width=20, height=2).grid(row=0, column=0, padx=5, pady=5)
...
def generate_frame(root, title, labels):
    ...
    for label_text in labels:
        ...
        entry = tk.Entry(inner_frame, width=8)  # Set width to 15
...
left_frame = generate_frame(root, "Delete vehicle:", ["ID:"])
left_frame.grid(row=2, column=0)
middle_left_frame = generate_frame(root, "Change battery:", ["ID:"])
middle_left_frame.grid(row=2, column=1)
middle_right_frame = generate_frame(root, "Move vehicle:", ["ID:", "Move:"])  # Added "Move:" label and its input box
middle_right_frame.grid(row=2, column=2)
right_frame = generate_frame(root, "Track vehicle:", ["ID:"])
